...
@implementation WYAttributeModel

@end


@implementation UILabel (WYAttributeTextTapAction)
...
- (id<WYAttributeTextDelegate>)delegate
{
    return objc_getAssociatedObject(self, _cmd);
}
...
- (void)setDelegate:(id<WYAttributeTextDelegate>)delegate
{
    ...
    for (CFIndex i = 0; i < count; i++) {
        ...
        if (CGRectContainsPoint(rect, point)) {
            ...
            for (int j = 0; j < link_count; j++) {
                ...
                WYAttributeModel *model = self.attributeStrings[j];
...
- (void)getRangesWithStrings:(NSArray <NSString *>  *)strings
{
    ...
    [strings enumerateObjectsUsingBlock:^(NSString * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (range.length != 0) {
            ...
            WYAttributeModel *model = [WYAttributeModel new];
