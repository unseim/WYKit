...
@interface WYDBHelper ()

@property (nonatomic, retain) FMDatabaseQueue *dbQueue;

@end

@implementation WYDBHelper

static WYDBHelper *_instance = nil;
...
+ (NSString *)dbPathWithDirectoryName:(NSString *)directoryName
{
    ...
    if (directoryName == nil || directoryName.length == 0) {
        docsdir = [docsdir stringByAppendingPathComponent:@"WYBD"];
    } else {
...
- (BOOL)changeDBWithDirectoryName:(NSString *)directoryName
{
    ...
    _instance.dbQueue = [[FMDatabaseQueue alloc] initWithPath:[WYDBHelper dbPathWithDirectoryName:directoryName]];
    ...
    if (numClasses >0 )
    {
        ...
        for (int i = 0; i < numClasses; i++) {
            if (class_getSuperclass(classes[i]) == [WYDBModel class]){
                id class = classes[i];
...
{
    return [WYDBHelper shareInstance];
}
