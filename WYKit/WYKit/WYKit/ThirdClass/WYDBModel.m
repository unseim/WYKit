...
@implementation WYDBModel

#pragma mark - override method
+ (void)initialize
{
    if (self != [WYDBModel self]) {
        [self createTable];
    }
}
...
{
    __block BOOL res = NO;
    WYDBHelper *jkDB = [WYDBHelper shareInstance];
...
//+ (BOOL)createTable
//{
//    FMDatabase *db = [FMDatabase databaseWithPath:[WYDBHelper dbPath]];
//    if (![db open]) {
//        NSLog(@"数据库打开失败!");
//        return NO;
//    }
...
{
    //判断是否是JKBaseModel的子类
    for (WYDBModel *model in array) {
        if (![model isKindOfClass:[WYDBModel class]]) {
            return NO;
        }
    }
...
+ (NSArray *)findAll
{
    ...
    [jkDB.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        while ([resultSet next]) {
            WYDBModel *model = [[self.class alloc] init];
            for (int i=0; i< model.columeNames.count; i++) {
